...
public class EditCourier extends JPanel {
	 ...
	 * Create the panel.
	 */
	public EditCourier(JFrame currentFrame) {
		...
		JLabel lblCustomerNo = new JLabel("Courier no         : ");
		lblCustomerNo.setBounds(45, 92, 135, 14);
		add(lblCustomerNo);
		
		JLabel lblNewLabel_2 = new JLabel("Courier Address:");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try{
					String query ="select * from courier where number=? ";
					Main log= new Main();
					PreparedStatement pst= log.conn(query);
					ResultSet rs=pst.executeQuery("select * from courier where number ='"+lblNewLabel_3.getText()+"'");
				
			    while(rs.next()){
			    	
			    	String query1 = "Update courier set couriername='"+textField.getText()+"',  courierphone='"+textField_5.getText()+"' , couriercol='"+textField_2.getText()+"', courieremail='"+textField_4.getText()+"'  ";
					PreparedStatement pst1= log.conn(query1);
					
					pst1.execute();
					JOptionPane.showMessageDialog(null, "Courier Details updated");
					pst1.close();
